...
struct sim_car_t : car_t
{
    ...
    sim_car_t(const car_t& c) : car_t{c} {}
...
struct street_t
{
    ...
    bool operator<(const street_t& that) const { return name < that.name; }
          ...
          os(o)
    {
        for (int time = 0; time < T_max; ++time)
        {
            ...
            for (auto no_id : busy_nodes)
            {
                ...
                {
                    os << "car " << car.id
                       << " finished moving to end_list on street " << new_st.id
                       << " " << new_st.name << '\n';
